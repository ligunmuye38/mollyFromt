...
const CaseItem = ({ title, content, percent, picUrl, name, price, type, className, isHover = false, isRotate = true, hoverContent, isOpen = true, backTheme, isActive = false, onClick }: CaseItemProps) => {
            ...
            <div className={clsx(cls.item_box, backTheme && cls[backTheme])}>
                {
                    ...
                    <>
                        {
                            isOpened == true &&
                            <div className={clsx("w-[133px] h-[153px] rounded-[12px] p-[1px] relative overflow-hidden", isActive ? "bg-[#10AA7C]" : "bg-[#161C28]")}>
                                <div className={clsx("w-full h-full rounded-[12px] p-[13px] flex flex-col justify-between relative overflow-hidden", cls.item_shadow, cls.item, className)}>
