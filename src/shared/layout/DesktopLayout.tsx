	...
	return (
		<div className='layout-main relative z-[0] grid grid-cols-[auto_1fr_auto] lg:block lg:pb-[70px]'>
			<div className='lg:layout-main__inner lg:grid-rows[auto-1fr]'>
				<div className='layout-main relative grid grid-cols-[auto_1fr_auto]'>
					<Sidebar />
					{itemPosition == 'left' && pathname != '/' && <HeaderLiveFeed className='h-screen w-[100px]' />}
					<div
						className='app-scrollbar h-screen overflow-y-auto lg:h-auto lg:overflow-visible'
					...
					{isVisible == true && (
						<div
							onClick={topScroll}
							className='top-scroll absolute bottom-3 right-3 flex min-h-[40px] min-w-[40px] items-center justify-center rounded-full hover:cursor-pointer'
						>
							<IconScrollTop className='h-[30px] w-[30px]' />
						</div>
					)}

					{itemPosition == 'right' && pathname != '/' && <HeaderLiveFeed className='h-screen w-[100px]' />}
				</div>
			</div>
			<Chat />
			<ChatButton />
