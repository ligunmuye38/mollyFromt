	...
	return (
		<div className={clsx(classNames?.base, isActive ? classNames?.baseVisible : classNames?.baseInvisible)}>
			<Button
				...
				classNames={{
					base: [
						classNames?.headerBase,
						isActive ? classNames?.headerBaseVisible : classNames?.headerBaseInVisible,
						'w-full justify-between'
					],
					content: classNames?.headerContent
				}}
				endContent={<div className={clsx(classNames.icon, isActive && classNames.iconVisible)}>{headerIcon}</div>}
			>
				<span
					className={clsx(
						classNames?.headerTitle,
						isActive ? classNames?.headerTitleVisible : classNames.headerTitleInVisible,
						'w-full justify-between'
					)}
				>
			...
			{line == true && <div className={clsx('flex h-0 w-full bg-[#1F2739]', isActive && 'h-[1px]')}></div>}
