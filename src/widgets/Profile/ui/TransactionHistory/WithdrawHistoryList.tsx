...
export const WithdrawHistoryListItem = ({ item }: WithdrawHistoryListItemProps) => {
	...
	return (
		<div
			onClick={onClick}
			className='flex h-12 cursor-pointer items-center gap-1 rounded-lg bg-[#111620] p-3'
		>
			<p className='w-1/5 md:w-[30%] md:text-[12px]'>{item.id}</p>
			<p className={'flex w-1/5 items-center gap-2 md:w-[40%] md:text-[12px]'}>
				{item.method.icon && (
			...
			<p className={'w-1/5 text-[#24FDBC] md:w-[30%] md:text-[12px]'}>{item.amount}</p>
			<p className='w-1/5 md:!hidden'>{item.date}</p>
	...
	return (
		<>
			<div className='mb-2 flex w-full gap-1 rounded-lg bg-[#1F2536] px-[15px] py-[9px] font-primary-bold text-[10px] text-[#5A6786]'>
				<span className='flex w-1/5 items-center justify-start uppercase md:w-[30%]'>
					{t('transaction_history_profile.id')}
				</span>
				<span className='flex w-1/5 items-center justify-start uppercase md:w-[40%]'>
					{t('transaction_history_profile.method')}
				</span>
				<span className='flex w-1/5 items-center justify-start uppercase md:w-[30%]'>
					{t('transaction_history_profile.amount')}
				</span>
				<span className='flex w-1/5 items-center justify-start uppercase md:!hidden'>
					{t('transaction_history_profile.date')}
				</span>
				<span className='flex w-1/5 items-center justify-end uppercase md:!hidden'>
