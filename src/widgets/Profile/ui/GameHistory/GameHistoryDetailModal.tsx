...
const GameHistoryDetailModal = ({ item }: { item: IUpgradeItem }) => {
		...
		<div>
			<div className='flex w-[calc(100vw_-_80px)] flex-col text-[12px]'>
				<div className='flex items-center justify-between border-b-2 border-b-[#161C28] py-[7px]'>
					<span className='text-[#5A6786]'>{t('game_history_profile.items_amount').toUpperCase()}</span>
					{item.status ? (
						...
						<p className={clsx(cls.game_history_upgrade_price)}>
				...
				<div className='flex justify-between border-b-2 border-b-[#161C28] py-[7px]'>
					<span className='text-[#5A6786]'>{t('game_history_profile.total_price').toUpperCase()}</span>
				...
				<div className='flex items-center justify-between border-b-2 border-b-[#161C28] py-[7px]'>
					<span className='text-[#5A6786]'>{t('game_history_profile.result').toUpperCase()}</span>
					<div className='flex items-center gap-2'>
						{item.status ? (
							<>
								<div className='flex h-[42px] w-[42px] items-center justify-center rounded-lg bg-[#111620]'>
				...
				<div className='flex justify-between py-[7px]'>
					<span className='text-[#5A6786]'>{t('game_history_profile.date_of_opening').toUpperCase()}</span>
					<p className='text-white'>{item.dateOfOpening}</p>
				</div>
			</div>
		</div>
	)
...
export default GameHistoryDetailModal
